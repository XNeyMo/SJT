...
fn generate_combinations<'a>(subjects: &'a [Subject], current_index: usize, current_combination: &mut Vec<&'a Course>, combinations: &mut Vec<Vec<&'a Course>>) {
    ...
    for course in &subjects[current_index].course {
        if current_combination.iter().all(|c| !conflicts_with(course, c)) {
            current_combination.push(course);
            generate_combinations(subjects, current_index + 1, current_combination, combinations);
...
fn main() {
    ...
    if let Some(best_combination) = combinations.into_iter().min_by_key(|comb| calculate_points(comb)) {
    ...
    (1..=s).map(|_| {
        ...
        for _ in 1..=c {
            ...
            for _ in 0..=d {
                ...
                space.push(Space { day, start_time, ending_time });
            }
